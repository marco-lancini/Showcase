...
def disconnect(request, backend, association_id=None):
    ...
    # It's a social login, not a custom profile
    if str(u.password) == "!":
        # If is last connected account, delete it
        if len(connected_accounts) == 0:
            return HttpResponseRedirect('/users/%s/destroy/' % str(request.user))
    ...
    #return HttpResponseRedirect(url)
    return HttpResponseRedirect('/users/%s/settings/' % str(request.user))
...
def complete_process(request, backend, *args, **kwargs):
    ...
    if user:
        if getattr(user, 'is_active', True):
            ...
            # user.social_user is the used UserSocialAuth instance defined in authenticate process
...
def auth_complete(request, backend, user=None, *args, **kwargs):
    ...
    if request.session.get(name):
        ...
        idx, args, kwargs = backend.from_session_dict(data, user=user, request=request, *args, **kwargs)
